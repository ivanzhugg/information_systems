...
class Supplier:
    ...
    def __str__(self) -> str:
        return (
            f"Поставщик #{self.__supplier_id}: {self.__name}; "
            f"адрес: {self.__address}; телефон: {self.__phone}"
        )
...
class SupplierShort:
    ...
    @classmethod
    def from_supplier(cls, supplier: Supplier) -> "SupplierShort":
        if not isinstance(supplier, Supplier):
            raise TypeError("Ожидается объект Supplier")
        return cls(supplier.supplier_id, supplier.name, supplier.phone)
...
if __name__ == "__main__":
    ...
    print("Одинаковый ID, разные контакты:", supplier == same_supplier)
    print("Разные ID:", supplier == other_supplier)
    print("Один и тот же объект:", supplier is same_supplier)
    try:
        supplier.phone = "123"
    ...
    print("ID краткого объекта:", short_supplier.supplier_id)
    supplier.name = "Новое название"
    print("Краткий объект хранит свою копию данных:", short_supplier)
